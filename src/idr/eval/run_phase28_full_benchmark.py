"""Phase 28: Definitive Authentic GNSS Blackout Benchmark Engine (180 Scenarios).

Executes comprehensive, leakage-free dead reckoning evaluation across authentic IO-VNBD dataset
drives (Validation Driver B / M and Held-Out Test Driver D / Y1) across 10s, 30s, and 60s
outage durations and 5 distinct vehicle motion regimes across 6 standardized baselines.
"""
...
logger = logging.getLogger("Phase28Benchmark")
...
def run_single_window_benchmark(
    ...
    for k, idx in enumerate(range(start_sim, end_sim)):
        ...
        if not is_blackout:
            ...
            if idx == bo_start - 1:
                pre_bo_pos = es_ekf.p.copy() if use_es_ekf else legacy_ekf.x[0:3].copy()
                pre_bo_vel = es_ekf.v.copy() if use_es_ekf else legacy_ekf.x[3:6].copy()
        else:
            if baseline == "const_vel_baseline":
                delta_t = (idx - bo_start + 1) * dt
                pred_pos = pre_bo_pos + pre_bo_vel * delta_t
                if es_ekf is not None:
                    es_ekf.p = pred_pos.copy()
                    es_ekf.v = pre_bo_vel.copy()
            else:
                if baseline in ("15state_ai_nhc", "15state_full", "legacy_ekf_full"):
                    if use_es_ekf and es_ekf is not None:
                        es_ekf.update_nhc(sigma_lat=0.05, sigma_vert=0.05)
                    elif use_legacy_ekf and legacy_ekf is not None:
                        apply_nhc_update(legacy_ekf, sigma_lat=0.05, sigma_vert=0.05)
                        
                if baseline in ("15state_ai", "15state_ai_nhc", "15state_full", "legacy_ekf_full"):
                    if ai_ready[k]:
                        ai_req_count += 1
                        val_speed = float(ai_speeds[k])
                        if use_es_ekf and es_ekf is not None:
                            accepted, metrics = es_ekf.update_ai_velocity(
                                val_speed,
                                sigma_v=0.8,
                                max_innovation_sigma=4.0,
                            )
                        elif use_legacy_ekf and legacy_ekf is not None:
                            accepted, metrics = legacy_ekf.update_ai_velocity(
                                val_speed,
                                sigma_v=0.8,
                                max_innovation_sigma=4.0,
                            )
                        ...
                        if accepted:
                            ...
                            ai_rej_count += 1
                        innov_val = metrics.get("innovation", 0.0)
                        if isinstance(innov_val, (list, tuple, np.ndarray)):
                            ai_innov = float(innov_val[0]) if len(innov_val) > 0 else 0.0
                        else:
                            ai_innov = float(innov_val)
                        ai_nis = float(metrics.get("nis", 0.0))
        ...
        if use_es_ekf and es_ekf is not None:
            ...
            pos_cov_tr = float(np.trace(legacy_ekf.P[0:3, 0:3]))
            vel_cov_tr = float(np.trace(legacy_ekf.P[3:6, 3:6]))
        ...
        spd_gt = float(np.linalg.norm(gt_vel[k, :2]))
        yaw_err_deg = float(np.rad2deg(min(abs(est_yaw[k] - gt_yaw[k]), 2 * np.pi - abs(est_yaw[k] - gt_yaw[k]))))
...
def execute_phase28_benchmark(
    ...
    """Execute complete Phase 28 Blackout Benchmark across validation and held-out test drives."""
    set_seed(42)
    ...
    logger.info("=" * 80)
    logger.info("PHASE 28: FULL AUTHENTIC 180-SCENARIO SCIENTIFIC BENCHMARK")
    logger.info("=" * 80)
    ...
    for win in all_windows:
        for b_name in baselines:
            ...
            if df_trace is not None:
                trace_filename = f"trace_p28_{win.drive_id}_{int(win.duration_s)}s_{win.regime}_{b_name}.csv"
    ...
    json_path = output_path / "PHASE28_FULL_AUTHENTIC_BENCHMARK.json"
